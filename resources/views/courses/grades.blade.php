      ...
      <div class="content">
        ...
        <div class="Home-content">
          ...
          @if(auth()->user()->type=='Professor'||auth()->user()->type=='admin')
            <div class="upload">
            <form  action="{{ route('storegrads') }}" method="POST"autocomplete="off" enctype="multipart/form-data">
                @csrf
                <h2>Upload File</h2>
                <input class="main-input" name='name'  type="text" placeholder="File Name" />
                <input id="fi"   name='file' type="file" />
                <label class="main-input" for="fi">
                    File <i class="fa-solid fa-file-pdf"></i>
                </label>
                <div class="btn">
                    <i></i>
            ...
            @if(count($grades)!=0)
                ...
                <div class="one-file">
                    <div>
                        <a href="{{ route('downloadgrads', $grade->id) }}"> <i class="fa-solid fa-download"></i> </a>
                    </div>
                    @if(auth()->user()->type=='admin')
                    <div style="position:absolute; right:20px;" id="delete_detecting_{{$grade->id}}" class="iconescom">
                    ...
                    <span>{{Carbon\Carbon::parse($grade->created_at)->format('d m Y')}}</span>
                        ...
                        @if($size>1024)
                            <span>{{floor($grade->size/1024)}}m</span>
                        @endif
                        @if($size<=1024)
                            <span>1m</span>
                        @endif
                ...
                <div class="conatiner conatinercom" id="delete_confirm_inbox{{ $grade->id }}">
                <form id="delete_programmer_form_{{ $grade->id }}"  action="{{route('deletegrads',$grade->id)}}" autocomplete="off">
                    <div class="login-title">
                        <h2>Are you sure?</h2>
                    </div>
                    <div class="page-box">
                        <div class="btn">
                            <input onclick="closePopupc('delete_confirm_inbox{{ $grade->id }}')" value="Cancel" id="cancel" class="form-btn" />
