      ...
      <div class="content">
        ...
        <div class="Home-content">
            ...
            @if(auth()->user()->type=='Professor')
                <div class="upload">
                    <form  action="{{ route('storelectures',$id) }}" method="POST"autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        <h2>Upload File</h2>
            ...
            @endif
                <div class="files">
                    ...
                    @php
                        $user=DB::table('users')->find($lecture->d_id)
                    @endphp
                    <div class="one-file">
                    <div>
                    <a href="{{ route('downloadlecture', $lecture->id) }}"> <i class="fa-solid fa-download"></i> </a>
                    </div>
                    @if(auth()->user()->id==$lecture->d_id)
                    <div style="position:absolute; right:20px;" id="delete_detecting_{{$lecture->id}}" class="iconescom">
                    ...
                    @endif
                        ...
                        <div class="info">
                            <span>{{Carbon\Carbon::parse($lecture->created_at)->format('d m Y')}}</span>
                            @if($lecture->size > 1024)
                            <span>{{floor($lecture->size/1024)}}m</span>
                            @endif
                            @if($lecture->size < 1024)
                                <span>1m</span>
                            @endif
                    ...
                    <div class="conatiner conatinercom" id="delete_confirm_inbox{{ $lecture->id }}">
                <form id="delete_programmer_form_{{ $lecture->id }}"  action="{{route('deletelecture',$lecture->id)}}" autocomplete="off">
                    <div class="login-title">
                        <h2>Are you sure?</h2>
                    </div>
                    <div class="page-box">
                        <div class="btn">
                            <input onclick="closePopupc('delete_confirm_inbox{{ $lecture->id }}')" value="Cancel" id="cancel" class="form-btn" />
