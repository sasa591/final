          ...
          <div class="responsive-table">
            <table>
              ...
              <tbody>
                ...
                @php
                $uu=DB::table('users')->find($object->usr_id)
                @endphp
                <tr>
                  <td> {{$uu->first_name}} {{$uu->last_name}} </td>
                  <td> {{$uu->code}}</td>
                  <td> {{date('Y-m-d',strtotime($object->created_at->addHours(3)))}}</td>
                  <td>{{$uu->year}}</td>
                  ...
                  <td>
                    <a href="{{route('object_admin_replay',$object->id)}}"><span>Show</span></a>
                  </td>
                </tr>
                @endforeach
              </tbody>
