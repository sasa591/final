        ...
        <div class="objections">
          ...
          <div class="responsive-table">
            <table>
              ...
              <tbody>
                @foreach($permenancydocuments as $permenancydocument)
                @php
                $uu=DB::table('users')->find($permenancydocument->usr_id)
                @endphp
                <tr>
                  <td> {{$uu->first_name}} {{$uu->last_name}} </td>
                  <td> {{$uu->code}}</td>
                  <td> {{date('Y-m-d',strtotime($permenancydocument->created_at->addHours(3)))}}</td>
                  <td>{{$permenancydocument->year}}</td>
                  <td>{{$permenancydocument->chapter}}</td>
                  <td>
                    <span class="label">{{$permenancydocument->status}}</span>
                  </td>
                  <td>
                    <a href="{{route('permanency_admin_replay',$permenancydocument->id)}}"><span>Show</span></a>
                  </td>
                </tr>
                @endforeach
              </tbody>
