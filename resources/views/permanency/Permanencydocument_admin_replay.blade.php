      ...
      <div class="content">
        ...
        <div class="replay-objection">
          <div class="main-heading">
            <h2>Permanency document Handling</h2>
          </div>
          <div class="content">
            <form action="{{route('store_replay_permanency',$ob->id)}}" method="post" autocomplete="off" id="objform">
            @csrf
            ...
            <div class="text">
               ...
               <bdi>{{$uu->first_name}} {{$uu->last_name}}</bdi> انا الطالب
                ...
                @php
                    ...
                    $carbon=Carbon::now();
                @endphp
                <bdi>{{$carbon->format('Y')}} {{$carbon->format('Y')-1}}</bdi>أرجو التفضل بالموافقة على منحي وثيقة دوام للعام الدراسي

             <br>
             @if($ob->chapter=='first_chapter')
                    الفصل الأول
            @endif
            @if($ob->chapter=='second_chapter')
                    الفصل الثاني
            @endif
            </div>
            <div class="image">
