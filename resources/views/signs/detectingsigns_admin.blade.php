        ...
        <div class="objections">
          ...
          <div class="responsive-table">
            <table>
              ...
              <tbody>
                @foreach($detectingsigns as $detectingsign)
                @php
                $uu=DB::table('users')->find($detectingsign->usr_id)
                @endphp
                <tr>
                  <td> {{$uu->first_name}} {{$uu->last_name}} </td>
                  <td> {{$uu->code}}</td>
                  <td> {{date('Y-m-d',strtotime($detectingsign->created_at->addHours(3)))}}</td>
                  <td>{{$uu->year}}</td>
                  <td>{{$detectingsign->student_type}}</td>
                  <td>
                    <span class="label">{{$detectingsign->status}}</span>
                  </td>
                  <td>
                    <a href="{{route('detectingsigns_admin_replay',$detectingsign->id)}}"><span>Show</span></a>
                  </td>
                </tr>
                @endforeach
              </tbody>
