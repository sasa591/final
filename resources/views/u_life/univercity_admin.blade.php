          ...
          <div class="responsive-table">
            <table>
              ...
              <tbody>
                ...
                @php
                $uu=DB::table('users')->find($document->usr_id)
                @endphp
                <tr>
                  <td> {{$uu->first_name}} {{$uu->last_name}} </td>
                  <td> {{$uu->code}}</td>
                  <td> {{date('Y-m-d',strtotime($document->created_at->addHours(3)))}}</td>
                  <td>{{$uu->year}}</td>
                  <td>{{$document->student_type}}</td>
                  <td>
                    <span class="label">{{$document->status}}</span>
                  </td>
                  <td>
                    <a href="{{route('univercity_admin_replay',$document->id)}}"><span>Show</span></a>
                  </td>
                </tr>
                @endforeach
              </tbody>
